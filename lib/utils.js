...
function getControllerDir(isInstall) {
  ...
  if (controllerDir[controllerDir.length - 3] == "adapter") {
    ...
    controllerDir = controllerDir.join("/");
    if (
      fs.existsSync(`${controllerDir}/node_modules/${appName}.js-controller`)
    ) {
...
function getConfig() {
  ...
  if (fs.existsSync(`${controllerDir}/conf/${appName}.json`)) {
    ...
    fs.existsSync(`${controllerDir}/conf/${appName.toLowerCase()}.json`)
  ) {
    return JSON.parse(
      fs.readFileSync(`${controllerDir}/conf/${appName.toLowerCase()}.json`)
    );
  }
