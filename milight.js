        ...
        if (adapter.config.version === "6") {
                ...
                if (!err) {
                  ...
                  if (dp === "on") {
                    ...
                    adapter.setForeignState(
                      ...
                      true,
                      true
                    ); //Nachführung von state
                  }
                  if (dp === "off") {
                    adapter.setForeignState(id, false, true); //Taste auf 0 setzen
                    adapter.setForeignState(
                      id.replace(".off", ".state"),
                      false,
                      true
                    ); //Nachführung von state
                  }
          ...
          if (dp === "colorMode") {
            if (
              ...
              adapter.log.debug(
                `V5 Send to zone ${zone} color Mode with hue=55`
              );
              if (
                !checkMethod(zones[zone], "on") ||
                !checkMethod(zones[zone], "hue")
              ) {
                return;
              }
              light
              ...
              if (
                !checkMethod(zones[zone], "on") ||
                !checkMethod(zones[zone], "whiteMode")
              ) {
                return;
              }
              light
            ...
            if (
              ...
              if (
                adapter.config.v5onFullBright === "true" ||
                adapter.config.v5onFullBright === true ||
                adapter.config.v5onFullBright === "on" ||
                adapter.config.v5onFullBright === "ON" ||
                adapter.config.v5onFullBright === 1
              ) {
                if (
                  !checkMethod(zones[zone], "on") ||
                  !checkMethod(zones[zone], "brightness")
                ) {
                  return;
                }
                light
...
function main() {
  adapter.config.commandRepeat =
    parseInt(adapter.config.commandRepeat, 10) || 2;
